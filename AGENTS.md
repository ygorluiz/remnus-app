...
- **Browser-style tabs (Tauri desktop only)** — Notion-like top tab strip. **Only rendered inside the Tauri shell** (web is unchanged — users open OS browser tabs there). Gated via `src/lib/hooks/useIsTauri.ts` (`'__TAURI_INTERNALS__' in window` check, SSR-safe).
  - `src/components/providers/TabsContext.tsx` — `TabsProvider` (client) holding `tabs: Tab[]` (`{id, href, title, icon, iconColor}`; `href` normalized to `/page/<id>` · `/db/<id>` · `/db/<id>/<pageId>`) + `activeId`. `useTabs()` returns `null` outside the provider (so the sidebar's new-tab hook no-ops on web). Active tab is **derived from `usePathname()`**: a matching tab activates; otherwise the active tab **navigates in place** (browser model) — single sidebar click follows the active tab, no duplicate. `openInNewTab(href, metaHint?)` creates a placeholder tab (tracked by `pendingNewTabRef`) then `router.push`; the next navigation resolves the placeholder's final href (so `+` → `/app` collapses into the redirect target, no stray tab). Meta for `/page` & `/db` tabs resolves **live** from the `items` prop (renames reflect); DB-row (`/db/x/y`) titles come from `document.title` via a `<title>` `MutationObserver` (editors already set it). Persisted to `localStorage['remnus_tabs_<workspaceId>']` (per workspace; deleted items pruned on load). Provider is **keyed by `activeWorkspaceId`** in `AppShell` so switching workspaces loads that workspace's own tab set.
  - `src/components/features/TabBar.tsx` — the strip (rendered in `AppShell` below `TauriTitlebar`). Per tab: `PageIcon` + title + hover × close; left-click activates, middle-click closes, right-click menu (Close / Close others / Close all), HTML5 drag-to-reorder (`reorderTabs`). Trailing `+` button opens a new tab (`/app`). i18n keys in `Layout`: `tabNewTooltip`/`tabClose`/`tabCloseOthers`/`tabCloseAll`/`tabUntitled`.
  - `src/components/AppShell.tsx` — takes `items` + `activeWorkspaceId`; in Tauri wraps the shell in `TabsProvider` and renders `<TabBar />`. New-tab entry: `WorkspaceSidebar` item `Link`s intercept Ctrl/Cmd+click and middle-click → `tabs.openInNewTab(hrefFor(item), …)`.
