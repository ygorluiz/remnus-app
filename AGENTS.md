...
- `CodeBlockExtension` (`FencedCodeBlock`) — extends `@tiptap/extension-code-block`, overriding only `renderMarkdown` to size the markdown fence to `max(3, longestInnerBacktickRun + 1)` backticks. The default code-block serializer always emits a 3-backtick fence, so a code body containing a ``` run closed the block early on reparse and corrupted the round-trip. Disabled in StarterKit (`codeBlock: false`) and added separately so it owns codeBlock serialization.
